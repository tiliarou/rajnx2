...
bool Utils::IsHblTid(u64 _tid) {
    return _tid == static_cast<u64>(sts::ncm::TitleId::AppletPhotoViewer);
}
...
static bool HasOverrideKey(const OverrideKey& cfg) {
    u64 kDown = 0;
...
bool Utils::HasOverrideButton(u64 tid) {
	...
	if (IsHblTid(tid) && HasOverrideKey(ParseOverrideKey("!R"))) {
		return true;
	}

	return HasOverrideKey(GetTitleOverrideKey(tid));
}
